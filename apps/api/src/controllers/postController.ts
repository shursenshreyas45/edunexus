...
export async function createPost(req: Request, res: Response) {
    try {
        ...
        const bodyTags = rawTagArray.map((t: string) => t.replace(/^#/, '').toLowerCase());
        ...
        const result = await pool.query(
            `INSERT INTO posts (author_id, title, content, metadata)
             VALUES ($1, $2, $3, $4)
             RETURNING id, author_id as "authorId", title, content, metadata, created_at as "createdAt", updated_at as "updatedAt"`,
            [authorId, title, content, metadata]
        );
    ...
    try {
        ...
        const result = await pool.query(
            `INSERT INTO comments (post_id, author_id, content)
             VALUES ($1, $2, $3)
             RETURNING id, post_id as "postId", author_id as "authorId", content, created_at as "createdAt", updated_at as "updatedAt"`,
