...
async function getProfile(req, res) {
    try {
        ...
        const result = await db_1.pool.query(`SELECT p.user_id as "id", p.full_name as "full_name", p.school_name as "school_name", p.batch_year as "batch_year", p.bio as "bio", u.tier
             FROM profiles p
...
async function upsertProfile(req, res) {
    try {
        ...
        const result = await db_1.pool.query(`INSERT INTO profiles (user_id, full_name, school_name, batch_year, bio)
             VALUES ($1, $2, $3, $4, $5)
             ON CONFLICT (user_id) 
             DO UPDATE SET 
             ...
             RETURNING full_name, school_name, batch_year, bio`, [userId, full_name, school_name, batch_year, bio]);
        res.status(200).json(result.rows[0]);
    }
    catch (error) {
